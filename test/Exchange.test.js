...
contract('Exchange', ([deployer, feeAccount, user1, user2]) => {
  ...
  beforeEach(async () => {
    ...
    // Deploy exchange
    exchange = await Exchange.new(feeAccount, oracleAddress)
  })
  ...
  describe('deployment', () => {
    ...
    it('getting fee', async () => {
      const result = await exchange.calcFee()
      result.logs[0].args.fee.toString().should.eq(fee)
    })
  })
  ...
  describe('order actions', () => {
    ...
    describe('filling orders', () => {
      ...
      describe('success', () => {
        beforeEach(async () => {
          // user2 fills order
          result = await exchange.fillOrder('1', { from: user2, value: fee})
        })
        ...
        it('executes the trade', async () => {
          ...
          balance.toString().should.equal(tokens(1).toString(), 'user2 tokens deducted')

          // checking fee
          balance = await exchange.balanceOf(ETHER_ADDRESS, feeAccount)
          balance.toString().should.equal(fee, 'feeAccount received Ether')
        })
        ...
        it('emits a "Fee" event', () => {
          const log = result.logs[0]
          log.event.should.eq('Fee')
          const event = log.args
          event.fee.toString().should.eq(fee, 'fee is correct')
          event.timestamp.toString().length.should.be.at.least(1, 'timestamp is present')
        })

        it('emits a "FeePaid" event', () => {
          const log = result.logs[1]
          log.event.should.eq('FeePaid')
          const event = log.args
          event.fee.toString().should.eq(fee, 'fee is correct')
          event.user.should.eq(user2, 'user is correct')
        })
      ...
      describe('failure', () => {
        const tooBigFee = '1000000000000001'
        const tooLessFee = '999999999999999'

        it('rejects too big fee', () => {
          exchange.fillOrder('1', { from: user2, value: tooBigFee}).should.be.rejectedWith(EVM_REVERT)
        })

        it('rejects too less fee', () => {
          exchange.fillOrder('1', { from: user2, value: tooLessFee}).should.be.rejectedWith(EVM_REVERT)
        })

        it('rejects invalid order ids', () => {
          const invalidOrderId = 99999
          exchange.fillOrder(invalidOrderId, { from: user2, value: fee}).should.be.rejectedWith(EVM_REVERT)
        })

        it('rejects already-filled orders', () => {
          // Fill the order
          exchange.fillOrder('1', { from: user2, value: fee}).should.be.fulfilled
          // Try to fill it again
          exchange.fillOrder('1', { from: user2, value: fee}).should.be.rejectedWith(EVM_REVERT)
        })
  ...
  describe('fillOrder()', () => {
    describe('Check balances after filling user1 buy Tokens order', () => {
      beforeEach(async () => {
        ...
        await exchange.fillOrder('1', {from: user2, value: fee})
      })
      ...
      it('feeAccount balance should should eq. fee', async () => {
        await (await exchange.balanceOf(ETHER_ADDRESS, feeAccount)).toString().should.eq(fee.toString())
      })
    })
    ...
    describe('Check balances after filling user1 buy Ether order', () => {
      beforeEach(async () => {
        ...
        // user2 deposit 1 ETHER to the exchange
        await exchange.depositEther({from: user2, value: ether(1)})
        // user2 fills the order
        await exchange.fillOrder('1', {from: user2, value: fee})
      })
